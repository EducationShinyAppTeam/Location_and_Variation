...
server <- function(input, output,session) { 
  ...
  observeEvent(input$inst1,{
    sendSweetAlert(
      ...
      title = "Instruction:",
      type = "info",
      print('1. Select the Location, Variation, or Random challenge to start the game.\n2. Create points by clicking in the plot to add points and watch your results until you have 15 points total.\n3. Show summary statistics as you go along by checking the corresponding box.\n4. There will be a message showing your results on the top of the plot.\n5. Click Clear Points and click New Challenge to start over')
    )
  })
  ...
  output$clusterPlot1 <- renderPlot({
    ...
    tryCatch({
      ...
      mclust2Dplot(data = store, what = "classification", 
                   classification = fit$classification, main = FALSE,
                   xlim=c(-0.1,5), ylim=c(0,10),cex=input$opt.cex, cex.lab=input$opt.cexaxis)
    }, error=function(warn){
      # Otherwise just plot the points and instructions
      plot(store, xlim=c(1, 10), ylim=c(0, 15), xlab="X", ylab="Frequency",
           cex=2.5, cex.axis=1.5,pch=16) 
      
      #if( input$median   == "TRUE" & length(val$x) >= 1 ){
      output$meanvalue <- renderText({
        if (length(val$x) < 15 & length(val$x) >= 1  & input$median   == "TRUE" ){
      ...
      if (length(val$x) < 15 & input$mean == "TRUE" & length(val$x) > 1){
        abline(v=(signif(mean(val$x),digits=3)),col="red",lwd = "4")
      }
      else if (length(val$x) >= 15 & input$mean == "TRUE")
      {
        abline(v=(signif(mean(store1[,1]),digits=3)),col="red",lwd = "4")
      }
      ...
      output$mean2 <- renderText({
        if(length(val$x) >= 2 & length(val$x) < 15 & input$mean == "TRUE" & input$median   == "TRUE"){
          paste("Absolute Difference:",signif(abs( mean(val$x)-median(val$x)),digits = 3))
        }
        else if (length(val$x) >= 15 & input$mean == "TRUE" & input$median   == "TRUE" )
        {
          paste("Absolute Difference:",signif(abs( mean(store1[,1])-median(store1[,1])),digits = 3))
        }
        
      })
      output$mean3 <- renderText({
        if(length(val$x) >= 1 & length(val$x) < 15 & input$mean == "TRUE" & input$median   == "TRUE"){
          paste("Absolute Difference:",signif(abs( mean(val$x)-median(val$x)),digits =2))
        }
        else if (length(val$x) >= 15 & input$mean == "TRUE" & input$median   == "TRUE")
        {
          paste("Absolute Difference:",signif(abs( mean(store1[,1])-median(store1[,1])),digits =2))
        }
      })
      ...
      if(length(val$x) < 15 & input$sd == "TRUE" & length(val$x) > 1){
        segments(mean(val$x)-sd(val$x),0,mean(val$x),0,lwd = "4", col="red")
        points.default(mean(val$x),0,pch=16,cex=2,col="red")
        segments(mean(val$x),0,mean(val$x)+sd(val$x),0,lwd = "4", col="red")
        
      }
      else if (length(val$x) >= 15 & input$sd == "TRUE") {
        
        segments(mean(store1[,1])-sd(store1[,1]),0,mean(store1[,1]),0,lwd = "4", col="red")
        points.default(mean(store1[,1]),0,pch=16,cex=2)
        segments(mean(store1[,1]),0,mean(store1[,1])+sd(store1[,1]),0,lwd = "4", col="red")
      }
      
      # }
      #  if (input$median  == "TRUE" & length(val$x) > 2 ){
      
      if(length(val$x) < 15 & input$median  == "TRUE" & length(val$x) >= 2){
        abline(v=(signif(median(val$x),digits=2)),col="blue",lwd = "4")}
      else if (length(val$x)>=15 & input$median  == "TRUE"){
        abline(v=(signif(median(store1[,1]),digits=2)),col="blue",lwd = "4")}
      #}
      ...
      if(length(val$x) < 15 & input$iqr == "TRUE" & length(val$x) >= 2){
        ...
        segments(quantile(val$x, 1/4),0.5,median(val$x),0.5,lwd = "4", col="blue")
        points.default(median((val$x)),0.5,pch=8,cex=2,col="blue")
        segments(median(val$x),0.5,quantile(val$x, 3/4),0.5,lwd = "4", col="blue")
        
      }
      else if (input$iqr == "TRUE" & length(val$x) >= 15){
        segments(quantile(store1[,1], 1/4),0.5,median(store1[,1]),0.5,lwd = "4", col="blue")
        points.default(median((store1[,1])),0.5,pch=8,cex=2,col="blue")
        segments(median(store1[,1]),0.5,quantile(store1[,1], 3/4),0.5,lwd = "4", col="blue")
      }
      ...
      output$diff1 <- renderText({
        if (c$right ==1 ){
          ...
          if (length(val$x) >= 15){
            ...
            if(signif(median(val$x),digits =2) < 1 && (signif(mean(val$x) > 4,digits = 2))){
              "Congratulations,you are right !"
            }
            else{
        ...
        if (c$right ==1 ){
          ...
          if (length(val$x) >= 15){
            if(sd(store1[,1]) < 1 && IQR(store1[,1]) < 1){
              ...
              else if (sd(store1[,1]) > 1 &&  IQR(store1[,1]) < 1 ){
          ...
          if (length(val$x) >= 15){
            if(signif(IQR(store1[,1]),digits = 2) == 0 && signif(sd(store1[,1]) > 1,digits = 2)){
              "Congratulations,you are right !"
            }
            else{
              "Sorry, Please click Clear Points and try again. \nHint: think about the variation of the points."            }
          ...
          if (length(val$x) >= 15){
            if(sd(store1[,1]) < 2 && IQR(store1[,1]) < 2){
              ...
              else if (sd(store1[,1]) > 2 &&  IQR(store1[,1]) < 2 ){
      ...
      output$diff3 <- renderText({
        if (d$right ==1 ){
          ...
          if (length(val$x) >= 15){
            if(signif(IQR(store1[,1]) == 0,digits = 1) && sd(store1[,1]) > 1){
              "Congratulations,you are right !"
            }
            else{
              "Sorry, Please click Clear Points and try again. \nHint: think about the variation of the points."            }
